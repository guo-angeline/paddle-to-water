...
**Slice shipped 2026-07-17 (commit 8538e40, DEPLOYED): the acute tide fast-fail from the verify-loop note below.** The conditions panel waits on both tide + wind (`getConditions` allSettled), so a hung tide call skeletoned the whole panel. Now the client (`fetchTides`) aborts the tide fetch at 4s and the `/api/tides` per-attempt timeout dropped 6s to 2.5s. Verified on prod: raw route ~5.5s (was up to 13s), client-capped tide wait ~4s. **Remaining for this item:** (1) collapse the NWS two-hop by precomputing the `/points` gridpoint per spot at build time; (2) a cached same-origin wind path (mirrors the item 52 tide proxy); (3) render wind independently of tide so a slow tide never skeletons the wind verdict (the deeper fix vs the 4s cap); (4) the `conditions_loaded` latency event already exists (`latency_ms`), so measure the before/after off it; (5) honesty bug surfaced during verification: on a tidal spot, a FAILED tide fetch renders "No tide station near this spot," which is false when a station exists but NOAA is down. Distinguish fetch-error from no-station-in-range.
