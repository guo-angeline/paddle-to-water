...
Four guardrails, none of them optional:

1. **Do not reuse a discredited screen, and do not call a source blocked until you have tried properly.** `sfbaywatertrail.org` 403s a bare `curl` but loads via WebFetch; Overpass 406s on GET but works on POST with a User-Agent. A blocked-source claim must survive both paths. The Cloudflare blocks on Marin, sccgov and EBRPD deeper pages still stand.
...
2. **The file shape is messier than any summary of it.** Corrected 2026-07-17: **37** records carry `id` as the LAST field with no trailing comma (not 28, as I first wrote), one record ends on `geocode_display`, there are **6 distinct key orders**, and some records now carry `owner_rating`. Never trust a shape count, including this one: any anchor-based edit must ASSERT its match count and fail loudly rather than silently skipping records.

3. **Precision is not accuracy, and a JSON round-trip is not a diff.** Never alter lat/lng without a verified source, and verify at the git-diff level, not numerically: `json.load` then `json.dump` silently reformats `-121.1729010` to `-121.172901`, so a float comparison reports "nothing changed" while the file churns. `git diff data/spots.json | grep -E '^[+-].*"(lat|lng)"'` must show ONLY the coordinates you intended to move. Prefer text-level edits.
4. **The 20% is not a leftover, it is `tide_sensitive`.** It gates `lib/savedConditions.ts`, i.e. the conditions engine, which is the app's differentiator. Verified 2026-07-17: **36 of 68 visible bay spots say `tide_sensitive: false`**. A wrong flag here means the app tells someone a tidal launch is not tide-dependent. If the 80% consumes the iteration, `tide_sensitive` gets split into its own `[ready]` item rather than deferred silently. It does not get dropped. **Do the tide pass FIRST**, before any coordinate work: it needs no external source, so doing it first makes this guardrail structurally impossible to lose.

   **CORRECTION 2026-07-17, and it is the point of this whole item.** I first wrote here that "14 records describe tides in their own notes while the flag says false" and reported that number to the owner as verified. It is wrong. My keyword screen (`tide|tidal` against `notes`) matched the word inside sentences asserting the OPPOSITE: **spot 96 (Lake Merced) says "free of tides and currents"** and **spot 60 says "Usable at all tide levels"**. Both are correctly `false` and must NOT be flipped. The real candidate set is ~12, and the screen is ~14% false-positive. Also note only 10 of the matches are `difficulty: bay`; 40 and 43 are river, 82 and 96 are flatwater. **Read all of them; flip none on a regex hit alone.** This is the fourth screen in this repo reported clean by its author and wrong at the edges (decimal-count 36%, DBW 75%, reverse-geocode 20%+, and now this one). The pattern is now the finding: a keyword screen cannot tell an assertion from its negation.
