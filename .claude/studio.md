...
notes: |
  ...
  - Adding ANY new spot trips the D19 predeploy gate (new spots.json lat/lng lines), so a new-spot deploy is always an escalation even when the coordinate is owner-supplied and verified. House pattern (D19): commit + merge to main so the coordinate diff is the review artifact, then escalate the DEPLOY (not the merge) and let the owner say go. Do not try to bypass the gate. COUPLING: once a gated coordinate sits on main undeployed, the gate blocks EVERY later deploy (it diffs deployed-prod vs the whole tree), so an unrelated fix merged after it cannot ship on its own until the owner clears the coordinate. Consequence: sequence coordinate-gated items LAST, or expect the next item's deploy to ride the same owner decision. Fold the later item into the open coordinate decision rather than opening a second one.
