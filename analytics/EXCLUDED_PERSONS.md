...
| `019f7cc0-eb59-7360-86ab-13080a457f23` | iOS Simulator / native app (posthog-react-native, `display_mode: native_ios`) | Claude's dev simulator: the 11-event verification burst that proved native analytics live (2026-07-19). The sim was flagged `ptw-internal` immediately after, so only these pre-flag events exist; any future native `display_mode: native_ios` person that isn't the owner's own iPhone is likely another dev simulator, verify and add. | 2026-07-19 |
