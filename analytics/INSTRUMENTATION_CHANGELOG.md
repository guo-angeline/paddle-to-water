...
- **`owner_rating: null` is not "unrated arm", it is "unrated spot".** 24 of 142 spots are deliberately blank because the owner has not paddled them. Use `owner_rating_shown` to tell arm/render from data absence. Never coerce a null rating to 0: it would read as a 0-of-5 verdict on a spot we simply have no opinion about.
...
**Read `reports/paddle-score-owner-ratings-2026-07-16.md` before reporting this experiment.** The ratings only discriminate in the North Bay (n=46, spread 1.9 vs. the pre-committed 1.5 threshold). All 29 East Bay ratings sit inside a 0.4-wide band, making treatment there close to a null. **A flat pooled result is the predicted outcome, not a finding.** Segment by region, and report the North-Bay-only read alongside the pooled one. The owner directed the full 118-spot ship on 2026-07-16 with this analysis in hand; the flat East Bay was known before launch, so do not later report it as a discovery.
