...
import { parseDeepLink, type DeepLink } from "../push/deepLink";
...
export default function HomeScreen({
  initialLink,
  fromSpotRoute = false,
}: { initialLink?: DeepLink; fromSpotRoute?: boolean } = {}) {
  ...
  // Deep links reach this screen three ways:
  //  1. Path form (paddletowater://spot/32) routes to src/app/spot/[id].tsx,
  //     which records the link and Redirects home -> onPendingLink delivers it.
  //  2. Query form (/?spot=32&from=alert&t=...) routes to THIS screen, so the
  //     params arrive as our own search params.
  //  3. A push-notification tap carries data.url (site-relative, the shape
  //     composeAlert writes) -> parseDeepLink -> same application path.
  function applyLink(link: DeepLink) {
    if (link.spotId === null) return;
  ...
  // Path-form deep links render the spot route, which passes the link in as a
  // prop (the native twin of web's <HomeClient initialSpotId>). Apply on mount.
  useEffect(() => {
    if (initialLink) applyLinkRef.current(initialLink);
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, []);
  const qp = useLocalSearchParams<{
    spot?: string | string[];
    from?: string | string[];
    t?: string | string[];
    window?: string | string[];
  }>();
  const qpSpot = Array.isArray(qp.spot) ? qp.spot[0] : qp.spot;
  useEffect(() => {
    if (!qpSpot || !/^\d+$/.test(qpSpot)) return;
    const one = (v: string | string[] | undefined) => (Array.isArray(v) ? v[0] : v);
    const from = one(qp.from);
    applyLinkRef.current({
      spotId: Number(qpSpot),
      from: from === "alert" || from === "share" || from === "email" ? from : "deeplink",
      token: one(qp.t) ?? null,
      windowLabel: one(qp.window) ?? null,
    });
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [qpSpot]);
  useEffect(() => {
    const sub = Notifications.addNotificationResponseReceivedListener((response) => {
      ...
      if (link) applyLinkRef.current(link);
    });
    return () => sub.remove();
  }, []);
  ...
  function deselect() {
    setSelected(null);
    setAlertBanner(null);
    // A deep-linked /spot/[id] route stacked a second copy of this screen on
    // top of index (or IS the cold-start root). Closing its sheet must leave
    // ONE live home screen, or taps hit a hidden duplicate underneath: replace
    // back to the root (the native twin of web's history rebase to "/").
    if (fromSpotRoute) router.replace("/");
  }
  ...
  return (
    <View style={styles.root}>
      ...
      {/* Full-screen spot detail (native twin of the web mobile sheet).
          Rendered OUTSIDE the SafeAreaView so it truly covers the whole
          screen, header and filter chrome included; the sheet's own app bar
          pads for the notch (mirrors web's inset:0 full-screen sheet). */}
      {selected && (
        <SpotSheet
          spot={selected}
          onClose={deselect}
