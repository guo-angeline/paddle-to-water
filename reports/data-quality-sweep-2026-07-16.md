...
*(Checked, since this would be serious: spots 76 and 79 were hidden on grounds that did **not** rest on DBW type alone — 76 on a byte-identical unsourced directory coordinate plus four independent negatives, 79 on the refuge/permit finding. Neither hide is undermined by this correction. Spot 120 was never hidden.)*
...
Highest-signal among them, by the audit's own criteria: **35** (Veterans Memorial Beach — reverse-geocodes to a house, no OSM launch within 5 km), **36** (Heart's Desire Beach), **29** (Corte Madera Creek — place/house centroid, 5.1 km from any launch), **37** (Lake Berryessa — administrative-boundary centroid, 5.1 km), **148** (Lakeshore Park, Newark — pin on a private residence at 35719 Scarborough Drive, 3.6 km from any launch; **worth checking whether boating is even permitted on this municipal lake**), **69** (Point Pinole), **31/33** (Russian River).
...
**3. Screen 2 covered only 38 spots** — those whose notes assert a ramp/launch/hoist. Records that assert *no* facility were not registry-checked. My spot→facility name matcher is fuzzy and produced 2 false positives out of 4 fires; **it should not be trusted unsupervised.**
...
1. **The defect rate did not fall when the sample got cleaner.** The coord audit sampled spots a heuristic flagged. The item 39 pilot sampled spots chosen for unrelated reasons and deliberately excluded every flagged spot — and still found 30–40%. This pass sampled a third way and completed 11 primary-source verifications; **all 11 had something wrong with them. Three independent sampling frames, three bad results.** That is a property of the corpus, not of the sampler.
...
- Run the Water Trail cross-check (finding 1). It is cheap, it is authoritative for the Bay Area, and it both **clears** correct records and **exposes** the ones that drifted from their source.
...
**Deep verification, 20 spots**, against agency pages, the DBW facility registry, SF Bay Water Trail site-description reports, CDFW, USFWS, CA State Parks, and OSM/Overpass geometry.
